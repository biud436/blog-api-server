...
export class CategoryService {
  ...
  async getTreeChildren(isBeautify: boolean): Promise<CategoryDepthVO[]> {
    ...
    for (const node of nodeList) {
      ...
      if (isRootNode(nodeVO)) {
        ...
        const ancestors = await this.getAncestorsWithDepth(visitied, nodeVO);
   ...
   * @param newCategoryId
   */
  async moveCategory({ newCategoryParentId, prevCategoryId }: MoveCategoryDto) {
    let isSuccess = false;
    ...
    try {
      ...
      const newNodeId = newNodeInsertResult.identifiers.map((e) => e.id)[0];
      ...
      if (!newNode) {
        throw new BadRequestException('새로운 노드를 찾을 수 없습니다.');
      }
